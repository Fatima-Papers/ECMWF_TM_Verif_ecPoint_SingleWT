...
###############################################################################################
# CODE DESCRIPTION
# 03_Compute_BSrel_AROCt_AROCz_BS.py computes the values of the Brier Score - Reliability component (BSrel), 
...
print(" ")
print("Computing BSrel, AROCt and AROCz, including " + str(RepetitionsBS) + " bootstrapped values")
...
# Computing the variables that will define the size for the BSrel, AROCt, and AROCz variables
m = len(StepF_list)
n = len(range(RepetitionsBS + 1))

# Computing BSrel, AROCt, and AROCz for a specific forecasting system
for indSystemFC in range(len(SystemFC_list)):
      ...
      # Computing BSrel, AROCt, and AROCz for a specific vre
      for vre in VRE_list:

            # Initializing the variable containing the BSrel, AROCt, and AROCz values, and the bootstrapped ones
            BSrel_array = np.zeros([m, n+1])
            AROCt_array = np.zeros([m, n+1])
            AROCz_array = np.zeros([m, n+1])

            # Computing BSrel, AROCt, and AROCz for a specific lead time
            for indStepF in range(len(StepF_list)):

                  # Selecting the StepF to consider
                  StepF = StepF_list[indStepF]
                  print(" - Computing BSrel, AROCt, and AROCz for " + SystemFC +", VRE>=" + str(vre) + ", StepF=" + str(StepF))
                  ...
                  # Computing BSrel, AROCt, and AROCz for the original and the bootstrapped values
            ...
            # # Saving AROCt
            # print("      - Saving AROCt for " + SystemFC + ", VRE>=" + str(vre))
            # DirOUT_temp = Git_repo + "/" + DirOUT + "/" + f"{Acc:02d}" + "h/AROCt/"
            # FileNameOUT_temp = "AROCt_" + f"{Acc:02d}" + "h_" + SystemFC + "_" + str(vre)
            # if not os.path.exists(DirOUT_temp):
            #       os.makedirs(DirOUT_temp)
